 ...
 * @LastEditTime: 2020-12-15 18:00:26
...
export class HandleFiles {
    public init({ sqlOption, libraryType = '', withLogger, withCache }, moduleNames?: string[]) {
        ...
        this.createFile('./server/router.ts', '../output/router.ts');
    ...
    public createModules(moduleNames: string[]) {
        for (let moduleName of moduleNames) {
            ...
            Promise.all([
                ...
                this.appendRouter(appendRouterFile, moduleName)
            ]);
        }
    }
    ...
    private appendRouter(buffer, moduleName) {
        let text = buffer.toString();
        text = text.replace(/\/\/ new module import here/g, `import ${moduleName}Router from './server/${moduleName}/router' \n new module import here`);
        text = text.replace(/\/\/ new module router here/g, `${moduleName}Router() \n new module router here`);
        const fileStream = fs.createWriteStream(path.resolve(__dirname, `../output/server/router.ts`));
        fileStream.on('open', () => {
            fileStream.write(text);
        });
        fileStream.on('error', (err) => { console.log(err) });
